...
<div class="editor-topbar wordpress-editor-topbar">
    ...
    <div class="actions editor-primary-actions">
        ...
        <a class="button" href="/admin/pages/builder/picker?path=<?= rawurlencode($identity->value()) ?>"><span
                    ...
                    <header class="builder-preview-toolbar">
                        ...
                        <div class="block-actions">
                            <button type="button" class="icon-button builder-action-move" data-block-move="-1"
                                aria-label="Przenieś blok wyżej" title="Przenieś wyżej">↑</button>
                            <button type="button" class="icon-button builder-action-move" data-block-move="1"
                                aria-label="Przenieś blok niżej" title="Przenieś niżej">↓</button>
                            <button type="button" class="icon-button builder-action-edit"
                                data-block-edit="<?= $escape($block['id']) ?>" aria-label="Edytuj blok"
                                title="Edytuj blok"><svg aria-hidden="true" viewBox="0 0 24 24" fill="none"
                            ...
                            <?php foreach ($actions as [$url, $fixedLabel, $class]):
                                ...
                                <form method="post" action="<?= $url ?>" <?= $class === 'builder-action-remove' ? ' data-confirm="Usunąć ten blok bezpowrotnie?"' : '' ?>><input type="hidden" name="_csrf"
                                        value="<?= $escape($csrfToken) ?>"><input type="hidden" name="identity"
                                        value="<?= $escape($identity->value()) ?>"><input type="hidden" name="id"
                                        value="<?= $escape($block['id']) ?>"><input type="hidden" name="revision"
                                        value="<?= $escape($revision->value()) ?>"><button type="submit"
                                        class="icon-button <?= $class ?>" aria-label="<?= $escape($label) ?>"
                                        title="<?= $escape($label) ?>">
                                        <?php if ($class === 'builder-action-toggle'): ?><svg aria-hidden="true"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                                stroke-width="2"><?php if ($block['enabled']): ?>
                                                    <path
                                                        d="M3 3l18 18M10.6 10.7a2 2 0 0 0 2.7 2.7M9.9 4.2A10.5 10.5 0 0 1 12 4c5 0 9 4 10 8a12.7 12.7 0 0 1-2.1 4.1M6.6 6.6A12.4 12.4 0 0 0 2 12c1 4 5 8 10 8 1.5 0 2.9-.4 4.1-1" />
                                                <?php else: ?>
                                                    <path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z" />
                                                    <circle cx="12" cy="12" r="3" /><?php endif; ?>
                                            </svg><?php elseif ($class === 'builder-action-duplicate'): ?><svg aria-hidden="true"
                                                viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                                <rect x="8" y="8" width="12" height="12" rx="2" />
                                                <path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2" />
                                            </svg><?php else: ?><svg aria-hidden="true" viewBox="0 0 24 24" fill="none"
                                                stroke="currentColor" stroke-width="2">
