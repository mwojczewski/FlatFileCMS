...
<label class="pages-search block-search">
    ...
    <input type="search" placeholder="Szukaj bloku…" data-block-search autocomplete="off">
</label>
        ...
        <a class="picker-card" data-block-card
            ...
            <?php else: ?>
                <span class="block-icon" aria-hidden="true">
                    <?= $blockIcon($definition->type()) ?>
                </span>
            <?php endif; ?>
            <span>
                <strong><?= $escape($card['name']) ?></strong>
                <small><?= $escape($card['description']) ?></small>
                <code><?= $escape($definition->type()) ?></code>
            </span>
        </a>
    <?php endforeach; ?>
</div>
<div class="empty-state block-library-empty" data-block-library-empty hidden>
    <strong>Nie znaleziono takiego bloku.</strong>
