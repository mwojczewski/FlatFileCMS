...
<details class="redirect-creator" <?= $rules === [] ? ' open' : '' ?>>
    <summary><span class="summary-plus" aria-hidden="true">＋</span><span><strong>Dodaj
                przekierowanie</strong><small>Połącz stary adres z nowym miejscem docelowym</small></span><span
            class="summary-chevron" aria-hidden="true">⌄</span></summary>
            ...
            <form class="redirect-form redirect-edit-form" method="post" action="/admin/redirects/update">
                ...
                <label>Kod<select name="status"><?php foreach ($redirectStatuses as $status => $description): ?>
                            <option value="<?= $status ?>" <?= $rule->status() === $status ? ' selected' : '' ?>><?= $status ?> —
                                <?= $escape($description) ?></option><?php endforeach; ?>
                    </select></label>
