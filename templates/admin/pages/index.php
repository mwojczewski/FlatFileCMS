        ...
        <tbody data-page-tree data-page-tree-csrf="<?= $escape($csrfToken) ?>">
            ...
            <?php foreach ($entries as $entry):
                ...
                <tr data-page-row data-page-identity="<?= $escape($identity) ?>" data-page-depth="<?= $depth ?>" data-page-collection="<?= $entry['collection'] ? '1' : '0' ?>" data-page-revision="<?= $escape($entry['revision']) ?>" data-page-search-value="<?= $escape(mb_strtolower($entry['title'] . ' ' . $identity)) ?>"<?= $entry['identity']->isHomepage() ? '' : ' draggable="true"' ?>>
                    ...
                    <td>
                        <span class="language-badges"><?php foreach ($languageCodes as $code): ?><span><?= $escape(strtoupper($code)) ?></span><?php endforeach; ?></span>
                    </td>
                    <td>
                    ...
                    <td><time class="page-modified" datetime="<?= $escape(date(DATE_ATOM, $entry['modifiedAt'])) ?>"><?= $escape(date('d.m.Y, H:i', $entry['modifiedAt'])) ?></time></td>
                    <td class="page-actions-column">
                        <details class="page-actions-menu" data-page-actions-menu>
                            <summary aria-label="Otwórz menu działań" title="Działania"><span aria-hidden="true">•••</span></summary>
                            <div class="page-actions-popover">
                            <?php if (!$entry['identity']->isHomepage()): ?>
                                <a href="/admin/pages/create?parent=<?= rawurlencode($identity) ?>"><span aria-hidden="true">＋</span>Dodaj podstronę</a>
                            <?php endif; ?>
                            <?php if (!$entry['collection']): ?><a href="/admin/pages/builder?path=<?= rawurlencode($identity) ?>"><span aria-hidden="true">▦</span>Otwórz edytor bloków</a><?php endif; ?>
                            <a href="<?= $entry['collection'] ? '/admin/collections/edit' : '/admin/pages/edit' ?>?path=<?= rawurlencode($identity) ?>"><span aria-hidden="true">✎</span>Edytuj ustawienia</a>
                            </div>
                        </details>
                    </td>
                </tr>
            <?php endforeach; ?>
            <tr data-page-empty hidden><td class="table-empty" colspan="6">Nie znaleziono strony pasującej do wyszukiwania.</td></tr>
        </tbody>
    </table>
</div>
