...
$render = $mapping($data['render'] ?? []);
$currentRenderEngine = is_string($render['engine'] ?? null) ? $render['engine'] : 'php';
...
<form class="stack crud-form editor-form" method="post" action="<?= $escape($action) ?>" <?= $creating ? ' data-canonical-suggest data-site-url="' . $escape($siteUrl) . '" data-canonical-base-path="' . $escape($canonicalBasePath) . '"' : '' ?>>
    ...
    <section class="form-section">
        <div class="section-heading">
            <div>
                <p class="eyebrow">Podstawowe</p>
                <h2>Ustawienia strony</h2>
            </div>
            <p>Widoczność, położenie i szablon dokumentu.</p>
        </div>
        <div class="settings-grid">
            <div>
                <?php if ($creating): ?><label>Ścieżka techniczna<input name="identity"
                            value="<?= $escape($currentIdentity) ?>" required placeholder="np. blog/nowy-wpis"></label>
                <?php else: ?><input type="hidden" name="identity" value="<?= $escape($currentIdentity) ?>">
                    <div class="technical-path"><span>Ścieżka techniczna</span><code><?= $escape($currentIdentity) ?></code>
                    </div><?php endif; ?>
            </div>
            ...
            <label>Sposób renderowania<select name="render_engine">
                    <option value="php" <?= $currentRenderEngine === 'php' ? ' selected' : '' ?>>PHP (layout i bloki)</option>
                    <option value="react-prerender" <?= $currentRenderEngine === 'react-prerender' ? ' selected' : '' ?>>React prerender</option>
                </select><small>React używa pliku public/app/prerender/{język}/{ścieżka}/index.html.</small></label>
    ...
    <section class="form-section">
        <div class="section-heading">
            <div>
                <p class="eyebrow">Języki</p>
                <h2>Treść i adresy</h2>
            </div>
            <p>Język domyślny jest wymagany. Puste tłumaczenie automatycznie użyje jego treści i sluga.</p>
        </div>
        <div class="locale-grid"><?php foreach ($languages->languages() as $locale => $name): ?>
                <fieldset class="form-card locale-card">
                    ...
                    <label>Tytuł strony<input name="title[<?= $escape($locale) ?>]"
                            value="<?= $escape($titleValues[$locale] ?? '') ?>" <?= $locale === $languages->default() ? ' required' : '' ?>></label>
                    <?php if (!$homepage): ?><label>Publiczny slug<input name="slug[<?= $escape($locale) ?>]"
                                value="<?= $escape($slugValues[$locale] ?? '') ?>" <?= $locale === $languages->default() ? ' required' : '' ?><?= $creating && $locale === $languages->default() ? ' data-public-slug' : '' ?>></label><?php endif; ?>
                    <label>Tytuł SEO<input name="seo_title[<?= $escape($locale) ?>]"
                            value="<?= $escape($seoTitles[$locale] ?? '') ?>"></label>
                    <label>Opis SEO<textarea name="seo_description[<?= $escape($locale) ?>]"
                            maxlength="500"><?= $escape($seoDescriptions[$locale] ?? '') ?></textarea></label>
                </fieldset>
            <?php endforeach; ?>
        </div>
    </section>
    <section class="form-section">
        <div class="section-heading">
            <div>
                <p class="eyebrow">Wyszukiwarki</p>
                <h2>Ustawienia SEO</h2>
            </div>
            <p>Canonical nowej strony zostanie podpowiedziany z URL witryny i domyślnego sluga.</p>
        </div>
        <fieldset class="form-card seo-card">
            ...
            <label>Canonical URL<input name="canonical" value="<?= $escape($canonical) ?>"
                    placeholder="https://example.com/ścieżka" <?= $creating ? ' data-canonical' : '' ?>></label>
            <div class="robots-grid"><label class="check"><input type="checkbox" name="robots_index" value="1"
                        <?= ($robots['index'] ?? true) === true ? ' checked' : '' ?>><span><strong>Pozwól
                            indeksować</strong><small>Wyszukiwarki mogą umieścić stronę w
                            wynikach.</small></span></label><label class="check"><input type="checkbox"
                        name="robots_follow" value="1" <?= ($robots['follow'] ?? true) === true ? ' checked' : '' ?>><span><strong>Pozwól śledzić linki</strong><small>Roboty mogą przechodzić do odnośników na
    ...
    <section class="danger-zone">
        <div class="section-heading">
            <div>
                ...
                <h2>Operacje na katalogu</h2>
            </div>
            <p>Te działania zmieniają fizyczną strukturę plików strony.</p>
        </div>
        <div class="danger-grid">
            <form class="danger-action" method="post" action="/admin/pages/move"><input type="hidden" name="_csrf"
                    value="<?= $escape($csrfToken) ?>"><input type="hidden" name="identity"
                    value="<?= $escape($currentIdentity) ?>"><input type="hidden" name="revision"
                    value="<?= $escape($revision) ?>">
                <h3>Przenieś stronę</h3>
                <p>Zmień ścieżkę techniczną bez utraty zawartości.</p><label>Nowa ścieżka techniczna<input
                        name="destination" value="<?= $escape($currentIdentity) ?>" required></label><button type="submit"
                    class="warning">Przenieś katalog</button>
            </form>
            <form class="danger-action delete-action" method="post" action="/admin/pages/delete"><input type="hidden"
                    name="_csrf" value="<?= $escape($csrfToken) ?>"><input type="hidden" name="identity"
                    value="<?= $escape($currentIdentity) ?>"><input type="hidden" name="revision"
                    value="<?= $escape($revision) ?>">
                <h3>Usuń stronę</h3>
                <p>Usunięty zostanie cały katalog wraz z podstronami i multimediami.</p><label>Wpisz <code>delete</code>,
                    aby potwierdzić<input name="confirmation" required autocomplete="off"></label><button type="submit"
                    class="danger">Usuń katalog bezpowrotnie</button>
            </form>
        </div>
    </section>
<?php endif; ?>
