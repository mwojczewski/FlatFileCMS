...
<div class="form-page-header">
    <div>
        <p class="eyebrow"><?= $editing ? 'Edycja dostępu' : 'Nowe konto' ?></p>
        <h2><?= $editing ? $escape($user->displayName()) : 'Dodaj administratora' ?></h2>
        <p class="lead">
            <?= $editing ? 'Zmień dane, hasło lub dostęp użytkownika do panelu.' : 'Utwórz konto z rolą administratora panelu.' ?>
        </p>
    </div>
    <div class="form-page-actions"><a class="button secondary" href="/admin/users">Wróć do administratorów</a></div>
</div>
<form class="stack crud-form editor-form account-editor-form" method="post"
    action="<?= $editing ? '/admin/users/update' : '/admin/users/create' ?>">
    ...
    <div class="actions form-actions"><span
            class="form-actions-context"><?= $editing ? 'Edytujesz istniejące konto' : 'Konto otrzyma rolę administratora' ?></span><a
            class="button secondary" href="/admin/users">Anuluj</a><button
