...
function mediaDialog() {
    ...
    dialog.innerHTML = '<div class="media-dialog-heading"><div><p class="eyebrow">Biblioteka strony</p>'
    ...
    dialog.addEventListener('click', (event) => {
        ...
        if (select instanceof HTMLButtonElement && typeof mediaSelection === 'function') {
            mediaSelection({
...
async function openMediaPicker(identity, kind, selection) {
    ...
    if (!(grid instanceof HTMLElement) || !(manage instanceof HTMLAnchorElement)) {
        return;
    }
    mediaSelection = selection instanceof HTMLInputElement
        ? (item) => {
    ...
    try {
        const response = await fetch(`/admin/media/picker?path=${encodeURIComponent(identity)}&kind=${encodeURIComponent(kind)}`, {
