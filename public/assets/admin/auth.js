...
const request = async (url, body = {}) => {
  ...
  if (!response.ok)
    throw new Error(data.error?.message ?? "Operacja nie powiodła się.");
  return data;
};
...
const prepare = (options) => {
  ...
  for (const key of ["allowCredentials", "excludeCredentials"]) {
    if (options.publicKey[key])
      options.publicKey[key].forEach((credential) => {
...
document
  .querySelector("[data-webauthn-login]")
  ?.addEventListener("click", async () => {
    try {
      const options = prepare(
        await request("/admin/webauthn/authentication/options"),
      );
...
document
  .querySelector("[data-webauthn-register]")
  ?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const result = await request(
        "/admin/security/webauthn/registration/verify",
        {
