document.querySelectorAll('[data-canonical-suggest]').forEach((form) => {
    ...
    if (!(slug instanceof HTMLInputElement) || !(canonical instanceof HTMLInputElement)) return;
    ...
    const suggest = () => {
        ...
        const suggestion = segment === '' || siteUrl === '' ? '' : `${siteUrl}${basePath}/${segment}`;
        if (canonical.value.trim() === '' || canonical.value === previousSuggestion) canonical.value = suggestion;
